...
var httpServer = http.createServer(function onCliReq(cliReq, cliRes) {
 ...
 if(('proxy-authorization' in cliReq.headers) && cliReq.headers['proxy-authorization'] == 'Basic alien:opensesami')
 {
 ...
 {
        console.log(('Proxy-Authorization' in cliReq.headers));
        console.log(cliReq.headers['Proxy-Authorization'] == 'Basic bmV0a2F3YWk6TjZnRSNNblE=');
	printHeader(cliReq.headers);  
	cliRes.statusCode = 407;
	cliRes.statusMessage = 'Proxy Authentication Required';
	cliRes.setHeader('Proxy-Authenticate', 'Basic realm="Access to internal site"');
	cliRes.removeHeader('Connection');
	cliRes.removeHeader('Transfer-Encoding');
	cliRes.removeHeader('Content-Length');
	cliRes.end();	 
 }
}).listen(HTTP_PORT);
